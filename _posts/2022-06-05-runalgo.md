...
Clone the GUI repository on GitHub and cd into the GUI directory. To run the GUI, use the following command.
...
**Note:** All commands must be terminated with a newline (\n) character. This is default in the Arduino serial monitor, but will need to be manually added if other serial interfaces are used.